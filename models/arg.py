...
class ARGModel(torch.nn.Module):
    def __init__(self, config):
        ...
        将新闻内容和 rationale（理由）转换为模型可以处理的表示形式。
        在 `ARGModel` 类中，使用了两个预训练的 BERT 模型：
        `self.bert_content`：用于处理新闻内容。
        `self.bert_FTR`：用于处理 rationale（FTR_2 和 FTR_3）。
        '''
        ...
        '''
        (c) Prediction
        使用 MaskAttention 和 MLP 来聚合新闻内容、rationale 交互特征和其他辅助特征，生成最终的分类结果。
        '''
        self.aggregator = MaskAttention(config['emb_dim'])
        self.mlp = MLP(config['emb_dim'], config['model']['mlp']['dims'], config['model']['mlp']['dropout'])

        '''
        (b-3) Rationale Usefulness Evaluation

        Hard MLP for FTR Usefulness：
        使用 `hard_mlp_ftr_2` 和 `hard_mlp_ftr_3` 来评估 rationale 的有用性。
        评估结果用于辅助任务，帮助模型更好地利用 rationale。
        '''
        ...
        '''
        (b-2) LLM Judgement Prediction

        Simple MLP for FTR Prediction：
        使用 `simple_mlp_ftr_2` 和 `simple_mlp_ftr_3` 来预测 rationale 的标签（如 "real" 或 "fake"）。
        预测结果用于辅助任务，帮助模型更好地理解 rationale 的内容。
        '''
    ...
    def forward(self, **kwargs):
        ...
        '''
        (b-1) News-Rationale Interaction
        新闻与理由的交互
        mutual_content_FTR_2 和 mutual_content_FTR_3 是通过交叉注意力机制计算的新闻内容与 rationale 的交互特征。
        '''
